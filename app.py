...
from PySide2.QtCore import Qt, QUrl, Slot, Signal, QThread, QObject
...
class Chain_Dialog(QtWidgets.QDialog):
    def __init__(self,parent=None):
        ...
        chain_label = QtWidgets.QLabel(json.dumps(blockchain.to_dict(), indent=4, sort_keys=True))
...
class Tx_Dialog(QtWidgets.QDialog):
    ...
    def allowButton(self):
        ...
        elif self.buttonBox.button(QtWidgets.QDialogButtonBox.Ok).isEnabled() == True:
            self.buttonBox.button(QtWidgets.QDialogButtonBox.Ok).setEnabled(False)

    def get_values(self):
        if self.tx_address and self.tx_amount:
            return {"address": self.tx_address.text(), "amount": int(self.tx_amount.text())}
        else:
            return False
...
class MyWidget(QtWidgets.QWidget):
    def __init__(self):
        ...
        self.address_value.setTextInteractionFlags(QtCore.Qt.TextSelectableByMouse)
        ...
        self.block_data.setTextInteractionFlags(QtCore.Qt.TextSelectableByMouse)
    ...
    def print_chain(self):
        if blockchain:
            ...
            msg.setText("No chain: you need to mine the first block or get the chain from another peer before seeing the chain")
    ...
    def send_tx(self):
        ...
        if blockchain:
            ...
            if ret_val == 1 and blockchain != None:
                tx_data = tx_window.get_values()
                self.define_tx(tx_data)
        else:
            msg = QtWidgets.QMessageBox()
            msg.setIcon(QtWidgets.QMessageBox.Warning)
            msg.setText("No chain: you need to mine the first block or get the chain from another peer before sending transactions")
            msg.setWindowTitle("No blockchain")
    ...
    def define_tx(self, elem):
        text = "Tx: { address : " + elem["address"] + ", amount : " + str(elem["amount"]) + " }" 
    ...
    def define_peer(self,elem):
        ...
        button_peer.clicked.connect(partial(self.remove_peer, elem, text_peer, button_peer))
